...
function Header({ darkMode, setDarkMode }: { darkMode: boolean; setDarkMode: (v: boolean) => void }) {
  ...
  // Ref to track if user clicked a nav link (suppress scroll-based detection briefly)
  ...
  const scrollTo = (i: number) => {
    ...
    // Re-enable scroll detection after smooth scroll settles (~800ms)
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // Scroll-position based section detection — most reliable approach
  ...
  // Recompute pill position — use RAF so scrolled nav buttons are painted first
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* ── Flat nav (visible when not scrolled) ── */}
      <motion.div
        className="flex items-center px-8 md:px-14 py-5 w-full"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg overflow-hidden shrink-0">
            <img src={faviconImg} alt="Joe Yoke logo" className="w-full h-full object-cover" />
          </div>
          <span className={`font-black text-sm tracking-tighter uppercase ${darkMode ? 'text-white' : 'text-[#1A1A1A]'}`}>
            Joe Yoke
          </span>
        </div>
        ...
        <nav className="flex-1 flex items-center justify-center gap-1">
          {NAV_LINKS.map((link, i) => (
            <button
              key={link}
              onClick={() => scrollTo(i)}
              className={`relative px-4 py-1.5 text-xs font-semibold tracking-wide rounded-full transition-colors duration-150 whitespace-nowrap ${
                activeNav === i
                  ? 'bg-[#C5FF00] text-[#1A1A1A]'
                  : darkMode
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-xs font-semibold transition-all ${
            darkMode ? 'text-white/50 hover:text-white' : 'text-[#1A1A1A]/55 hover:text-[#1A1A1A]'
          }`}
        >
      ...
      {/* ── Frosted pill nav (visible when scrolled) ── */}
      <motion.div
      ...
      >
        <div
          className={`flex items-center gap-2 px-4 py-2.5 rounded-full border transition-all duration-300 ${
            darkMode
              ? 'bg-[#111111]/90 backdrop-blur-md shadow-lg shadow-black/30 border-white/10'
              : 'bg-white/90 backdrop-blur-md shadow-lg shadow-black/5 border-[#E5E7EB]'
          }`}
        ...
        >
          ...
          <div className="flex items-center gap-2 mr-2">
            ...
            <span className={`font-black text-sm tracking-tighter uppercase transition-colors ${darkMode ? 'text-white' : 'text-[#1A1A1A]'}`}>
              Joe Yoke
            </span>
          </div>

          {/* Nav links with sliding pill — refs here drive pillStyle */}
          <nav className="flex-1 flex items-center justify-center">
            <div className="relative flex items-center gap-0.5">
              ...
              {NAV_LINKS.map((link, i) => (
                <button
                  key={link}
                  ref={(el) => { pillNavRefs.current[i] = el }}
                  onClick={() => scrollTo(i)}
          ...
          <button
            onClick={() => setDarkMode(!darkMode)}
            className={`ml-2 flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold transition-all ${
              darkMode
                ? 'border-white/15 text-white/50 hover:text-white hover:border-white/30'
                : 'border-[#E5E7EB] text-[#1A1A1A]/60 hover:text-[#1A1A1A] hover:border-[#1A1A1A]/20'
            }`}
          >
            {darkMode ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
            <span className="hidden sm:inline">{darkMode ? 'LIGHT' : 'DARK'}</span>
...
function Hero({ dark }: { dark: boolean }) {
  ...
  const subtext = get('hero', 'subtext')
  const ctaLabel = get('hero', 'ctaLabel')

  // Split headline at "the friendships" for lime coloring (works for custom text too)
  const limeWord = 'the friendships'
  const limeIdx = headline.toLowerCase().indexOf(limeWord)
  const headlineParts = limeIdx >= 0
    ? [headline.slice(0, limeIdx), headline.slice(limeIdx, limeIdx + limeWord.length), headline.slice(limeIdx + limeWord.length)]
    : [headline, '', '']
    ...
    <section className={`min-h-screen flex items-center px-6 md:px-12 pt-20 pb-16 transition-colors duration-500 ${dark ? 'bg-[#0A0A0A]' : 'bg-[#F8F9FA]'}`}>
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12 md:gap-8">
        ...
        <div className="flex-1 flex flex-col gap-8">
          ...
          >
            <h1 className={`text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-black leading-none tracking-tighter transition-colors duration-500 ${dark ? 'text-white' : 'text-[#1A1A1A]'}`}>
              {limeIdx >= 0 ? (
                <>{headlineParts[0]}<span className="text-[#C5FF00]">{headlineParts[1]}</span>{headlineParts[2]}</>
              ) : headline}
            </h1>
          </motion.div>

          <motion.p
            className={`text-lg max-w-md leading-relaxed transition-colors duration-500 ${dark ? 'text-white/50' : 'text-[#1A1A1A]/50'}`}
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            ...
            >
              <img
                src={heroSrc}
                alt="Hero mockup"
                className="w-72 h-72 sm:w-96 sm:h-96 lg:w-[480px] lg:h-[480px] object-contain drop-shadow-2xl select-none"
                draggable={false}
              />
            </motion.div>
          </motion.div>
  ...
  return (
    <section className={`py-20 px-6 md:px-12 transition-colors duration-500 ${dark ? 'bg-[#0A0A0A]' : 'bg-[#F8F9FA]'}`}>
      <div className="max-w-4xl mx-auto flex flex-col items-center gap-12">
        <FadeUp>
          <p className={`text-2xl md:text-3xl text-center leading-snug font-medium transition-colors duration-500 ${dark ? 'text-white/60' : 'text-[#1A1A1A]/70'}`}>
            {quote}
          </p>
        </FadeUp>
        ...
        <FadeUp delay={0.1}>
          <div className="flex flex-wrap items-center justify-center gap-3">
            {pills.slice(0, -1).map((word) => (
              <span
                key={word}
                className={`px-6 py-2.5 rounded-full border font-semibold text-sm tracking-wide transition-colors duration-500 ${
                  dark ? 'border-white/15 text-white' : 'border-[#1A1A1A]/20 text-[#1A1A1A]'
                }`}
              >
                {word}
              </span>
            ...
            {pills[pills.length - 1] && (
              <span className={`px-6 py-2.5 rounded-full border font-semibold text-sm tracking-wide transition-colors duration-500 ${dark ? 'border-white/15 text-white' : 'border-[#1A1A1A]/20 text-[#1A1A1A]'}`}>
                {pills[pills.length - 1]}
              </span>
            )}
          </div>
...
// Accent colors per index for landing-page cards (cycles if more than 4 games)
...
function GameCard({ game, delay, index }: { game: Game; delay: number; dark: boolean; index: number }) {
  ...
  return (
    <FadeUp delay={delay}>
      {/* Background card with accent glow — gives each card its own identity */}
      <div
        className="rounded-[28px] p-[3px] transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl"
        style={{
          background: `linear-gradient(135deg, ${accent}30 0%, transparent 60%)`,
          boxShadow: `0 0 0 1px ${accent}22, 0 24px 48px ${accent}15`,
        }}
      >
        <div className="group relative rounded-3xl overflow-hidden" style={{ minHeight: 320 }}>
          ...
          {/* Frosted glass description panel — bottom */}
          <div
            className="absolute bottom-0 left-0 right-0 z-10 flex flex-col gap-2 px-5 pb-5 pt-10"
            style={{
              background: 'linear-gradient(to bottom, transparent, rgba(8,8,10,0.72) 30%, rgba(8,8,10,0.88) 100%)',
              backdropFilter: 'blur(0px)',
              WebkitBackdropFilter: 'blur(0px)',
            }}
          >
            <div className="absolute inset-0 pointer-events-none" style={{ backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)', maskImage: 'linear-gradient(to bottom, transparent 0%, black 60%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, black 60%)' }} />
            <div className="absolute inset-0 pointer-events-none" style={{ backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', maskImage: 'linear-gradient(to bottom, transparent 25%, black 75%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 25%, black 75%)' }} />
            <div className="absolute inset-0 pointer-events-none" style={{ backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', maskImage: 'linear-gradient(to bottom, transparent 50%, black 90%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 50%, black 90%)' }} />
...
function TrendingGames({ dark }: { dark: boolean }) {
  ...
  const { get } = useContent()
  const sectionTitle = get('games', 'sectionTitle')
  // Featured games for landing page (first 4), fallback to static GAMES if context empty
  ...
  const displayGames: Game[] = featured.length > 0
    ? featured.map((g, i) => ({
        ...
        image: g.imageUrl || [gameImg1, gameImg2, gameImg3, gameImg4][i] || gameImg1,
      }))
    : GAMES
      ...
      <div className="max-w-7xl mx-auto">
        <FadeUp>
          <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
            <h2 className={`text-4xl md:text-5xl font-black tracking-tighter leading-none transition-colors duration-500 ${dark ? 'text-white' : 'text-[#1A1A1A]'}`}>
              {sectionTitle}
            </h2>
            <button
              onClick={() => navigate('/games')}
              className={`group flex items-center gap-2 text-sm font-semibold transition-colors ${dark ? 'text-white/40 hover:text-white' : 'text-[#1A1A1A]/50 hover:text-[#1A1A1A]'}`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {displayGames.map((game, i) => (
            <GameCard key={game.id} game={game} delay={i * 0.08} dark={dark} index={i} />
          ))}
        </div>
      </div>
    </section>
...
function CategoryRow({ cat, delay, dark }: { cat: Category; delay: number; dark: boolean }) {
  ...
  return (
    <FadeUp delay={delay}>
      <div
        className={`group flex items-center justify-between px-6 md:px-8 py-6 cursor-pointer transition-all duration-150 rounded-xl ${
          hovered ? 'bg-[#C5FF00]' : ''
        } ${dark ? 'border-t border-white/10 last:border-b' : 'border-t border-[#E5E7EB] last:border-b'}`}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      >
        <div className="flex items-center gap-6 md:gap-10 min-w-0">
          <span className={`text-sm font-mono font-bold shrink-0 transition-colors duration-150 ${hovered ? 'text-[#1A1A1A]/50' : dark ? 'text-white/30' : 'text-[#1A1A1A]/30'}`}>
            {cat.index}
          </span>
          <span className={`text-xl md:text-2xl font-black tracking-tight transition-colors duration-150 ${hovered ? 'text-[#1A1A1A]' : dark ? 'text-white' : 'text-[#1A1A1A]'}`}>
            {cat.title}
          </span>
          <span className={`hidden sm:block text-sm transition-colors duration-150 ${hovered ? 'text-[#1A1A1A]/60' : dark ? 'text-white/30' : 'text-[#1A1A1A]/40'}`}>
            {cat.tags}
          </span>
        </div>
        <div className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center border transition-all duration-200 ${
          hovered ? 'bg-[#1A1A1A] border-[#1A1A1A] scale-110' : dark ? 'border-white/20 bg-transparent' : 'border-[#1A1A1A]/20 bg-transparent'
        }`}>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col">
          {CATEGORIES.map((cat, i) => (
            <CategoryRow key={cat.index} cat={cat} delay={i * 0.07} dark={dark} />
          ))}
        </div>
      </div>
    </section>
...
function CommunityStats({ dark }: { dark: boolean }) {
  const { get } = useContent()
  const headlineRaw = get('stats', 'headline')
  const ctaLabel = get('stats', 'ctaLabel')
  const stats = [1, 2, 3, 4].map(n => ({
    value: get('stats', `stat${n}_value`),
    label: get('stats', `stat${n}_label`),
  }))
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-14">
              <div>
                ...
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-white tracking-tighter leading-none">
                  {headlineRaw.split('\\n').map((line, i) => (
                    <span key={i}>{line}{i < headlineRaw.split('\\n').length - 1 && <br />}</span>
                  ))}
                </h2>
              </div>
              ...
              {stats.map((stat, i) => (
                <FadeUp key={i} delay={i * 0.1}>
                  <div className="flex flex-col gap-2">
                    <span className="text-4xl md:text-5xl font-black text-[#C5FF00] tracking-tighter leading-none">{stat.value}</span>
...
function Footer({ dark }: { dark: boolean }) {
  const { get } = useContent()
  const ctaHeadline = get('footer', 'ctaHeadline')
  const ctaTagline = get('footer', 'ctaTagline')
  const ctaBtn = get('footer', 'ctaBtn')
        ...
        <FadeUp>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-16 border-b border-white/10">
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white tracking-tighter leading-none max-w-2xl">
              {ctaHeadline}{' '}
              <span className="text-[#C5FF00]">{ctaTagline}</span>
            </h2>
            ...
            {FOOTER_LINKS.map((col) => (
              <div key={col.heading} className="flex flex-col gap-4">
                ...
                <ul className="flex flex-col gap-2.5">
                  {col.links.map((link) => (
                    <li key={link}>
                      <a href="#" className="text-sm text-white/50 hover:text-white transition-colors duration-150">{link}</a>
                    </li>
                  ))}
                </ul>
              </div>
...
// Stacked blur layers — each covers 0-100% of the bar height, masked so only
// the bottom portion of each layer is visible. Higher layers = stronger blur.
// This produces a smooth eased progression from 0→max blur, matching the
// iOS / macOS "liquid glass" look.
const BLUR_LAYERS = [
  { blur: 1,  start: 0,   end: 16  },
  { blur: 2,  start: 8,   end: 28  },
  { blur: 4,  start: 18,  end: 42  },
  { blur: 8,  start: 30,  end: 58  },
  { blur: 12, start: 44,  end: 72  },
  { blur: 18, start: 56,  end: 84  },
  { blur: 24, start: 68,  end: 92  },
  { blur: 32, start: 78,  end: 100 },
]
...
function LiquidGlassBar({ dark }: { dark: boolean }) {
  ...
  // Anchor to bottom; top edge moves DOWN as user scrolls (blur shrinks upward)
  ...
  return (
    <div
      ...
      style={{ bottom: 0, height: `calc(200px - ${shift}px)` }}
    >
      {/* Progressive blur layers — no visible lines, pure backdrop blur */}
      ...
      {/* Subtle tint — fades in toward bottom only */}
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: `linear-gradient(to bottom, transparent 40%, ${tint} 100%)`,
