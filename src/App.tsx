  ...
  return (
    <motion.header className="fixed top-0 inset-x-0 z-50 pointer-events-none" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6, ease }}>
      ...
      <motion.div className="flex items-center justify-between px-4 sm:px-8 md:px-14 py-4 md:py-5 w-full" animate={{ opacity: scrolled ? 0 : 1 }} transition={{ duration: 0.25 }} style={{ pointerEvents: scrolled ? 'none' : 'auto' }}>
        ...
        {/* Hidden on mobile to prevent stretching */}
      ...
      <motion.div className="absolute top-2 md:top-4 inset-x-0 flex justify-center px-2" animate={{ opacity: scrolled ? 1 : 0, y: scrolled ? 0 : -8 }} transition={{ duration: 0.3 }} style={{ pointerEvents: scrolled ? 'auto' : 'none' }}>
        <div className={`flex items-center justify-between gap-1 md:gap-2 px-2 md:px-4 py-1.5 md:py-2.5 rounded-full border transition-all duration-300 ${darkMode ? 'bg-[#111111]/90 backdrop-blur-md shadow-lg shadow-black/30 border-white/10' : 'bg-white/90 backdrop-blur-md shadow-lg shadow-black/5 border-[#E5E7EB]'}`} style={{ maxWidth: 720, width: '100%' }}>
          ...
          <div className="flex items-center gap-1.5 md:gap-2 shrink-0 md:mr-2">
            ...
            {/* Text hidden on mobile to save space for nav links */}
  ...
  return (
    <section className={`min-h-screen flex items-center px-4 sm:px-6 md:px-12 pt-24 md:pt-20 pb-16 transition-colors duration-500 ${dark ? 'bg-[#0A0A0A]' : 'bg-[#F8F9FA]'}`}>
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12 md:gap-8 overflow-hidden">
        ...
        <div className="w-full md:w-3/5 lg:w-2/3 flex flex-col gap-6 md:gap-8 z-10 shrink-0">
          <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.9, ease, delay: 0.1 }}>
            {/* Adjusted typography scales for mobile */}
            <div className={`text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black leading-[1.1] tracking-tighter transition-colors duration-500 break-words [&_p]:m-0 ${dark ? 'text-white' : 'text-[#1A1A1A]'}`} dangerouslySetInnerHTML={renderHTML(get('hero', 'headline'))} />
          </motion.div>
  ...
  return (
    <section className={`py-16 md:py-20 px-4 sm:px-6 md:px-12 transition-colors duration-500 ${dark ? 'bg-[#0A0A0A]' : 'bg-[#F8F9FA]'}`}>
      <div className="max-w-4xl mx-auto flex flex-col items-center gap-10 md:gap-12">
        <FadeUp>
          {/* Adjusted typography scales for mobile */}
          <div className={`text-xl sm:text-2xl md:text-3xl text-center leading-snug font-medium transition-colors duration-500 [&_p]:m-0 ${dark ? 'text-white/60' : 'text-[#1A1A1A]/70'}`} dangerouslySetInnerHTML={renderHTML(get('about', 'quote'))} />
        </FadeUp>
        <FadeUp delay={0.1}>
      ...
      <div className="max-w-7xl mx-auto">
        <FadeUp>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 md:mb-12 gap-4">
            <div className={`text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter leading-none transition-colors duration-500 [&_p]:m-0 ${dark ? 'text-white' : 'text-[#1A1A1A]'}`} dangerouslySetInnerHTML={renderHTML(get('games', 'sectionTitle'))} />
            <button onClick={() => navigate('/games')} className={`group inline-flex items-center gap-2 text-xs md:text-sm font-semibold transition-colors w-fit ${dark ? 'text-white/40 hover:text-white' : 'text-[#1A1A1A]/50 hover:text-[#1A1A1A]'}`}>
      ...
      <div className={`group flex flex-col sm:flex-row sm:items-center justify-between px-4 sm:px-6 md:px-8 py-5 md:py-6 cursor-pointer transition-all duration-150 rounded-xl gap-4 sm:gap-0 ${hovered ? 'bg-[#C5FF00]' : ''} ${dark ? 'border-t border-white/10 last:border-b' : 'border-t border-[#E5E7EB] last:border-b'}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
        <div className="flex items-start sm:items-center gap-4 sm:gap-6 md:gap-10 min-w-0 flex-1">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-6 md:gap-10 min-w-0">
            <div className={`text-lg sm:text-xl md:text-2xl font-black tracking-tight transition-colors duration-150 [&_p]:m-0 ${hovered ? 'text-[#1A1A1A]' : dark ? 'text-white' : 'text-[#1A1A1A]'}`} dangerouslySetInnerHTML={renderHTML(cat.title)} />
            <div className={`text-xs md:text-sm transition-colors duration-150 [&_p]:m-0 ${hovered ? 'text-[#1A1A1A]/60' : dark ? 'text-white/30' : 'text-[#1A1A1A]/40'}`} dangerouslySetInnerHTML={renderHTML(cat.tags)} />
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        <FadeUp>
          <div className="mb-8 md:mb-10">
            <div className={`text-[10px] md:text-xs font-bold tracking-widest uppercase mb-2 md:mb-3 transition-colors duration-500 [&_p]:m-0 ${dark ? 'text-white/30' : 'text-[#1A1A1A]/40'}`} dangerouslySetInnerHTML={renderHTML(get('categories', 'subhead'))} />
            <div className={`text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter leading-none transition-colors duration-500 [&_p]:m-0 ${dark ? 'text-white' : 'text-[#1A1A1A]'}`} dangerouslySetInnerHTML={renderHTML(get('categories', 'headline'))} />
          </div>
        </FadeUp>
...
function CommunityStats({ dark }: { dark: boolean }) {
  ...
  return (
    <section id="community" className={`py-16 md:py-20 px-4 sm:px-6 md:px-12 transition-colors duration-500 ${dark ? 'bg-[#0A0A0A]' : 'bg-[#F8F9FA]'}`}>
      <div className="max-w-7xl mx-auto">
        <FadeUp>
          <div className={`rounded-3xl p-6 sm:p-10 md:p-14 border transition-colors duration-500 ${dark ? 'bg-[#111111] border-white/5' : 'bg-[#0A0A0A] border-white/5'}`}>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-10 mb-10 md:mb-14">
              <div>
                <p className="text-[10px] md:text-xs font-bold text-white/30 tracking-widest uppercase mb-3 md:mb-4">Our Community</p>
                <div className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white tracking-tighter leading-[1.1] [&_p]:m-0" dangerouslySetInnerHTML={renderHTML(get('stats', 'headline'))} />
              </div>
              ...
              {stats.map((stat, i) => (
                <FadeUp key={i} delay={i * 0.1}>
                  <div className="flex flex-col gap-1 md:gap-2">
                    <div className="text-3xl sm:text-4xl md:text-5xl font-black text-[#C5FF00] tracking-tighter leading-none [&_p]:m-0" dangerouslySetInnerHTML={renderHTML(stat.value)} />
                    <div className="text-[10px] md:text-sm text-white/40 font-medium [&_p]:m-0" dangerouslySetInnerHTML={renderHTML(stat.label)} />
                  </div>
                </FadeUp>
              ))}
            </div>
...
function Footer({ dark }: { dark: boolean }) {
  ...
  return (
    <footer id="download" className={`pt-16 md:pt-20 pb-0 px-4 sm:px-6 md:px-12 overflow-hidden transition-colors duration-500 ${dark ? 'bg-[#050505]' : 'bg-[#0A0A0A]'}`}>
      <div className="max-w-7xl mx-auto">
        <FadeUp>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8 pb-12 md:pb-16 border-b border-white/10">
            <div className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-black text-white tracking-tighter leading-none max-w-2xl flex flex-wrap gap-1.5 md:gap-2 [&_p]:m-0">
        ...
        <FadeUp delay={0.1}>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-10 py-10 md:py-14 border-b border-white/10">
            {FOOTER_LINKS.map((col) => (
              <div key={col.heading} className="flex flex-col gap-3 md:gap-4">
                ...
                <ul className="flex flex-col gap-2 md:gap-2.5">
                  {col.links.map((link) => (
                    <li key={link}><a href="#" className="text-xs md:text-sm text-white/50 hover:text-white transition-colors duration-150">{link}</a></li>
                  ))}
                </ul>
              </div>
            ))}
        ...
        <div className="flex items-center py-6 text-[10px] md:text-xs text-white/20 [&_p]:m-0">
...
export default function App() {
  ...
  return (
    // Crucial: overflow-x-hidden strictly prevents horizontal scrolling on mobile
