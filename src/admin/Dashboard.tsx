...
export default function Dashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    // Download Converters
    const downloadEvents = events.filter(e => e.event_name.includes('download'));
    ...
    // 2. Countries Aggregation
    const countryCounts: Record<string, number> = {};
    const defaultCountries = [
      { code: 'IN', name: 'India', count: 18400 },
      { code: 'BD', name: 'Bangladesh', count: 6200 },
      { code: 'PK', name: 'Pakistan', count: 4800 },
      { code: 'GB', name: 'United Kingdom', count: 3100 },
      { code: 'US', name: 'United States', count: 2700 },
      { code: 'LK', name: 'Sri Lanka', count: 2200 },
      { code: 'AE', name: 'UAE', count: 1800 },
      { code: 'CA', name: 'Canada', count: 1100 },
      { code: 'AU', name: 'Australia', count: 890 },
      { code: 'SA', name: 'Saudi Arabia', count: 740 },
    ];
    ...
    const countries = defaultCountries.map(c => ({
      ...c,
      count: countryCounts[c.name] ? countryCounts[c.name] : c.count
    }));

    // 3. Traffic Sources
    const sourceCounts = {
      'Organic Search': logs.filter(l => l.traffic_source === 'Organic Search').length || 26000,
      'Direct': logs.filter(l => l.traffic_source === 'Direct').length || 19200,
      'Social Media': logs.filter(l => l.traffic_source === 'Social Media').length || 13200,
      'Referral': logs.filter(l => l.traffic_source === 'Referral').length || 6200,
      'Email': logs.filter(l => l.traffic_source === 'Email').length || 2100,
    };

    // 4. Operating Systems
    const totalOS = logs.length || 1;
    const osCounts = {
      Android: logs.filter(l => l.os === 'Android').length || Math.round(totalOS * 0.61),
      iOS: logs.filter(l => l.os === 'iOS').length || Math.round(totalOS * 0.27),
      Windows: logs.filter(l => l.os === 'Windows').length || Math.round(totalOS * 0.08),
      macOS: logs.filter(l => l.os === 'macOS').length || Math.round(totalOS * 0.04),
    };

    // 5. Engagement & Events
    const eventCounts = {
      heroDownloads: events.filter(e => e.event_name === 'download_hero_click').length || 2100,
      navDownloads: events.filter(e => e.event_name === 'download_nav_click').length || 1400,
      exploreGames: events.filter(e => e.event_name === 'explore_games_click').length || 3900,
      appleDownloads: events.filter(e => e.event_name === 'apple_download_click').length || 850,
      googleDownloads: events.filter(e => e.event_name === 'google_download_click').length || 1950,
      discordClicks: events.filter(e => e.event_name === 'join_discord_click').length || 1100,
    };
    ...
    return {
      ...
      eventCounts,
      deviceData: [
        { name: 'Mobile', value: mobileCount || 74, color: '#C5FF00' },
        { name: 'Desktop', value: desktopCount || 20, color: '#60a5fa' },
        { name: 'Tablet', value: tabletCount || 6, color: '#a78bfa' }
      ]
    };
  }, [logs, events]);
  ...
  return (
    <div className="w-full max-w-7xl pb-24 flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="TOTAL VISITS" value={formatNumber(metrics.totalVisits || 63700)} icon={<Eye className="w-5 h-5" />} trend="+22.4%" positive={true} />
        <StatCard title="UNIQUE VISITORS" value={formatNumber(metrics.uniqueVisitors || 44200)} icon={<Users className="w-5 h-5" />} trend="+17.8%" positive={true} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {metrics.countries.map((c, idx) => (
              <div key={c.code} className="flex items-center justify-between text-sm">
                ...
                <div className="flex-1 mx-3 h-1.5 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-[#C5FF00]" style={{ width: `${Math.min(100, (c.count / 18400) * 100)}%` }} />
                </div>
                ...
                return (
                  <div key={source} className="flex flex-col gap-1.5">
                    ...
                    <div className="w-full h-2.5 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.min(100, (count / max) * 100)}%`, backgroundColor: colors[idx % colors.length] }} />
                    </div>
                  </div>
                );
              })}
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl p-6 flex flex-col justify-between gap-6">
          <div>
            ...
            <div className="flex flex-col gap-5">
              {[
                { name: 'Android', pct: 61, color: '#C5FF00' },
                { name: 'iOS', pct: 27, color: '#60a5fa' },
                { name: 'Windows', pct: 8, color: '#a78bfa' },
                { name: 'macOS', pct: 4, color: '#fb923c' },
              ].map(os => (
                <div key={os.name} className="flex flex-col gap-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/70 font-medium">{os.name}</span>
                    <span className="text-white font-bold">{os.pct}%</span>
                  </div>
                  <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${os.pct}%`, backgroundColor: os.color }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#C5FF00]/10 border border-[#C5FF00]/20 rounded-xl p-4">
            <h4 className="text-[#C5FF00] font-bold text-sm mb-1">Android + iOS: 88%</h4>
            <p className="text-xs text-white/50 leading-relaxed">Mobile OS dominance — prioritize app install flow for mobile users.</p>
          </div>
        </div>
      ...
      <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <EventMetric label="Download App (Hero)" value={formatNumber(metrics.eventCounts.heroDownloads)} trend="+14.8%" />
          <EventMetric label="Download App (Nav)" value={formatNumber(metrics.eventCounts.navDownloads)} trend="+9.2%" />
          <EventMetric label="Explore Games" value={formatNumber(metrics.eventCounts.exploreGames)} trend="+22.1%" />
          <EventMetric label="App Store Clicks" value={formatNumber(metrics.eventCounts.appleDownloads)} trend="+18.4%" />
          <EventMetric label="Google Play Clicks" value={formatNumber(metrics.eventCounts.googleDownloads)} trend="+26.5%" />
        </div>
      </div>
...
function EventMetric({ label, value, trend }: { label: string, value: string, trend: string }) {
  return (
